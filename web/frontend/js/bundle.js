...
/*#__PURE__*/
function () {
  function AudioSample(node, audioSamples) {
  ...
  _createClass(AudioSample, [{
    ...
    value: function init() {
      ...
      this.word = this.control.querySelector('.word'); // scroll
    ...
    value: function scroll() {
      var rect = this.footer.getBoundingClientRect();

      if (rect.top < window.innerHeight) {
        this.canvas.style.webkitTransform = "translateY(".concat(-(window.innerHeight - rect.top), "px)");
        this.canvas.style.MozTransform = "translateY(".concat(-(window.innerHeight - rect.top), "px)");
        this.canvas.style.msTransform = "translateY(".concat(-(window.innerHeight - rect.top), "px)");
        this.canvas.style.OTransform = "translateY(".concat(-(window.innerHeight - rect.top), "px)");
        this.canvas.style.transform = "translateY(".concat(-(window.innerHeight - rect.top), "px)");
      }
    }
  }, {
    key: "toggle",
    ...
    key: "stop",
    value: function stop() {
      this.audioSamples.forEach(function (item) {
        return item.pause();
      });
    }
  }, {
    ...
    value: function start() {
      ...
      function renderFrame() {
        ...
        for (var i = 0; i < bufferLength; i++) {
          barHeight = dataArray[i]; // var r = barHeight + (25 * (i / bufferLength));
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var audioSamples = document.querySelectorAll('.m-album__sample');
  var objects = [];
  audioSamples.forEach(function (sample) {
...
document.addEventListener('DOMContentLoaded', function () {
  var targets = document.querySelectorAll('[data-target="modal"]');
  targets.forEach(function (target) {
    var trigger = target.querySelector('a');
    var modal = target.querySelector('.m-modal');
...
require("./components/AudioSample");

},{"./components/AudioSample":1,"./components/Modal":2,"./components/Navigation":3,"./main":6,"./polyfills":7}],6:[function(require,module,exports){
"use strict";
...
document.addEventListener('DOMContentLoaded', function () {
  console.log('hello earth people'); // scroll to target on load
  ...
  }, 1); // scroll to target on on click
  ...
  var cards = [].slice.call(document.querySelectorAll(".m-card"));
  if (cards) (0, _functions["default"])(cards);
  var waves = document.querySelectorAll('.waves path');
  window.addEventListener('scroll', throttle(function () {
    elementFromBottom(waves, 'show', 100);
  }, 50), false);
}); // Source: http://fofwebdesign.co.uk/template/_testing/scroll-in-view/add-class-to-element-when-in-view.htm

function elementFromBottom(elem, classToAdd, distanceFromBottom) {
  elem.forEach(function (el) {
    var distTop = el.getBoundingClientRect().top;
    var distBottom = window.innerHeight - distTop;
...
},{"./functions":4,"swiper":8}],7:[function(require,module,exports){
"use strict";
